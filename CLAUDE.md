...
- **本地开发环境限制**：本仓库所在的这个开发环境`pip install`用不了（网络层SSLEOFError，详情见"环境信息"），所以上述所有"真实跑通"都是在另一台训练服务器（`ssh retinascope-server`）上做的，不是在这台机器上。模型权重文件（`models/essay-scorer-*/v1/pytorch_model.bin`，共约266MB）留在训练服务器的`/data/wangchen/tiansukai/RAG/models/`下，没有同步回这个仓库（`.gitignore`排除了`.bin`文件），仓库里只保留了体积小的`training_log.json`（训练过程/QWK指标）、真实数据生成的`data/kb/*.md`、`data/processed/score_ranges.json`、`data/processed/eda/*.png`这些小文件。**如果需要在别的地方实际跑`streamlit run app.py`并让评分是真实模型输出**，要么重新跑一遍`Docs/RUNNING.md`第3~5节的训练/建库流程，要么从训练服务器把`models/`和`data/processed/chroma_kb/`拷贝过来（`scoring_tool_node`/`retrieval_agent_node`在这些文件不存在时会自动降级回Day1的占位逻辑，不会报错崩溃）。
...
├── models/                    模型权重本身gitignore（太大），只保留了训练日志
